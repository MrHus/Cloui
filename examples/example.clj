...
(def act (action-listener :performed (greet event lbl txt)))
(def mse (mouse-listener :clicked (mouse-clicked event lbl) :released (mouse-clicked event lbl)))
(def ky  (key-listener :pressed (key-action event lbl)))
...
(def menuact (action-listener :performed (menu-action event lbl)))               
